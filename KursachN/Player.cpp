...
void Player::update(float dt)
{
	...
	int player_map_position_x = x / map->getCellSize();
	int player_map_position_y = y / map->getCellSize();
	...
	for (int i = 0; i < map->getColumnsCount(); i++) {
		for (int j = 0; j < map->getRowsCount(); j++) {
			...
			case Map::BLOCK_TYPE::FIRE:
				...
				curr_x = collider->getPosition().x / cell_size;
				curr_y = collider->getPosition().y / cell_size;
