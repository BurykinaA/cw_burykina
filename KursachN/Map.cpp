...
void Map::draw(sf::RenderWindow& window)
{
	...
	for (int i = 0; i < rows; i++) {
		for (int j = 0; j < columns; j++) {
			...
			sf::RectangleShape rect = sf::RectangleShape(sf::Vector2f(cell_size, cell_size));
			rect.setPosition(i * cell_size, j * cell_size);
			switch (current_block)
			{
			case Map::BLOCK_TYPE::GROUND:
				anim1.setTextureRect(sf::IntRect(32, 0, 32, 32));
				anim1.setPosition(i * cell_size, j * cell_size);
				window.draw(anim1);
			...
			case Map::BLOCK_TYPE::FIRE:
				anim2.setPosition(i * cell_size, j * cell_size);
				anim1.setTextureRect(sf::IntRect(32, 0, 32, 32));
				anim1.setPosition(i * cell_size, j * cell_size);
				window.draw(anim1);
				window.draw(anim2);
